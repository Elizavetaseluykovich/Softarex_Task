...
const ImageItem = ({url, src, photographer_url, photographer, id, width, height, location}) => {
    ...
    return (
        <Link key={id} to={{pathname: `/photo/${id}/`, state: {url, src, photographer, photographer_url, id, width, height, background: location, like: find(likes, id)}}} >
            <div className={styles.col}>
                <article>
                    <img src={src.original} alt={url.substring(29, url.lastIndexOf('-'))} className={styles.mainImg}/>
                        ...
                        <ul>
                            <li>
                                <a href={photographer_url} className={styles.author} target="_blank" rel="noreferrer" onClick={e => e.stopPropagation()}>
                                    <img className={styles.authorPic} src={src.small} alt={photographer}></img>
                                    <span>{photographer}</span>
                                </a>
                            </li>
                            <li>
                                <a className={styles.downloadIcon} target="_blank" rel="noopener noreferrer" onClick={() => {downloadPhoto(src.original, photographer)}}>
                                    <i>
